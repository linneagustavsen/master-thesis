...
'''
def calculationsRandomForestTelemetryEntropy(trainingSet, testingSet, systemId, frequency, interval, attackDate):
    p = Path('Calculations')
    ...
    if not q.exists():
        q.mkdir(parents=True)
    timeStamps = pd.read_pickle(str(q) + "/Entropy.Testing."+ str(int(interval.total_seconds())) +"secInterval.attack."+str(attackDate)+ "."+str(systemId)+ ".pkl")["_time"].to_numpy()
