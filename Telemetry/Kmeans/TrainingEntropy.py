...
def training(start, stop, systemId, if_name):
    ...
    df.to_pickle("Telemetry/Kmeans/Data/TrainingDataEntropy.pkl")
    #df = pd.read_pickle("Telemetry/Kmeans/data.pkl")  
    timeStamps, measurements = structureData(df)


    kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(measurements)
    pickle.dump(kmeans, open("Telemetry/Kmeans/Models/MLmodelEntropy.pkl", 'wb'))
