...
def detection(start, stop, systemId, if_name, fields):
    f0 = open("Telemetry/Kmeans/Data/Combined.Cluster0."+ str(systemId) + "." + str(if_name).replace("/","-") + ".csv", "a")
    f1 = open("Telemetry/Kmeans/Data/Combined.Cluster1."+ str(systemId) + "." + str(if_name).replace("/","-") + ".csv", "a")
    ...
    #df.to_pickle("Telemetry/Kmeans/Data/TestingDataEntropy.pkl")
    df = pd.read_pickle("Telemetry/Kmeans/Data/TestingDataCombined.pkl")
    measurements = df.values

    timeStamps = pd.read_pickle("Telemetry/Kmeans/Data/TestingData.pkl")["_time"].to_numpy()
    kmeans = pickle.load(open("Telemetry/Kmeans/Models/MLmodelCombined.pkl", 'rb'))
