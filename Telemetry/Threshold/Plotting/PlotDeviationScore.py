    ...
    for table in tables:
        for row in table.records:
            ...
            if deviation >= 1:
                ...
                ax.plot(x, distributionThisMinute.pdf(x), label="Normal distribution")
                ax.scatter([row.values["_value"]], [distributionThisMinute.pdf(row.values["_value"])], color="red", label="Probability")
                ax.annotate(str(distributionThisMinute.pdf(row.values["_value"])), xy=(row.values["_value"], distributionThisMinute.pdf(row.values["_value"])))
