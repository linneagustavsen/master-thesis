...
def correlateDetections(file1, file2, file3, file4, start, stop):
    detectionsFile = open("CorrelationTest/Detections " + str(start) + ".txt", "a")
    ...
    while not stop:
        ...
        if check(file1, timestamp.strftime('%Y-%m-%d %H:%M')) and check(file2, timestamp.strftime('%Y-%m-%d %H:%M')) and check(file3, timestamp.strftime('%Y-%m-%d %H:%M')) and check(file4, timestamp.strftime('%Y-%m-%d %H:%M')):
...
thesholdPacketsMaxVarFile = "ThresholdTelemetry/Detections/2022-09-21 01:00:00.trd-gw.xe-0-1-0.egress_stats__if_1sec_pkts.MaxVar.txt"

correlateDetections(entropyPacketSizeFile, entropyRatePacketSizeFile, thresholdPacketsFile, thesholdPacketsMaxVarFile, "2022-09-21 01:00:00", "2022-09-22 00:00:00")