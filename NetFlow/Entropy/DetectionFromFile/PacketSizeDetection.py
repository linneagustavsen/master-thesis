...
'''
def detectionPS(start, stop, systemId, frequency, interval, windowSize, thresholdPSEntropy, thresholdPSEntropyRate, weightPSEntropy, weightPSEntropyRate, attackDate):
    p = Path('NetFlow')
    ...
    data = pd.read_csv("Calculations"+fileString+"/Entropy/NetFlow/Metrics."+ str(int(interval.total_seconds())) +"secInterval.attack."+str(attackDate)+ "."+str(systemId)+ ".csv",
                       usecols=["sTime","eTime", "packetSizeEntropy", "packetSizeEntropyRate", "real_label"])
    ...
    attackScores = open(str(q) + "/ScoresAttacks.PacketSizeEntropy."+ str(int(interval.total_seconds())) +"secInterval.attack."+str(attackDate)+ "."+str(systemId)+ ".json", "w")
    json.dump(attackDict,attackScores)
    attackScores.close()

    attackScores = open(str(q) + "/ScoresAttacks.PacketSizeEntropyRate."+ str(int(interval.total_seconds())) +"secInterval.attack."+str(attackDate)+ "."+str(systemId)+ ".json", "w")
