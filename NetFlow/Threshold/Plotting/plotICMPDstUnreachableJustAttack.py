...
def makePlotICMPDstUnreachable(systemId, interval, attackDate):
    ...
    if attackDate == "08.03.23":
        timeAxis[0] = datetime.strptime("Mar 08 14:30:00", format).replace(year=2023)
    elif attackDate == "17.03.23":
        timeAxis[0] = datetime.strptime("Mar 17 11:00:00", format).replace(year=2023)
    elif attackDate == "24.03.23":
        timeAxis[0] = datetime.strptime("Mar 24 14:00:00", format).replace(year=2023)
    ...
    #fig.tight_layout()
    fig.savefig("Plots/Threshold/Attack"+ fileString+"/NetFlow/ICMPDstUnreachable/JustAttackPeriod/"+  str(systemId)+ "." + str("ICMPDstUnreachable")+ "."+ str(int(interval.total_seconds())) +"secInterval.pdf", dpi=300)
    plt.close(fig)
