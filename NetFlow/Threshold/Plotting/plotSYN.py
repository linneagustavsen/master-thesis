...
def plotSYN(systemId, attackDate):
    ...
    axs.set_title("SYN packets per flow", fontsize=20)
    axs.set_xlabel('Time',fontsize=20)
    axs.set_ylabel("SYN packets per flow", fontsize=20)
    axs.tick_params(axis='both', which='major', labelsize=15)
    fig.legend(fontsize=20)
    fig.tight_layout()
    fig.savefig("Plots/Threshold/Attack"+ fileString+ "/NetFlow/SYN/OnlySYNAttack.Scatter."+  str(systemId)+ ".SYN.png", dpi=300)
    plt.close()
