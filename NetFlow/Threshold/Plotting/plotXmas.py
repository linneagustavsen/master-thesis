...
def plotSYN(systemId, attackDate):
    ...
    data = pd.read_csv("/Users/linneafgustavsen/Desktop/Masteroppgave/FromAttack23/Calculations/Threshold/NetFlow/Xmas.attack."+str(attackDate)+ "."+str(systemId)+ ".csv")
    ...
    start = datetime.strptime(strings[7][0], format).replace(year=2023)
    stop = datetime.strptime(strings[7][1], format).replace(year=2023)
    axs.axvspan(start, stop, facecolor=colors[7], label=attacks[7])
    ...
    axs.set_title("FIN flag set")
    axs.set_xlabel('Time')
    axs.set_ylabel("FIN flag set")
    axs.tick_params(axis='both', which='major', labelsize=12)
    fig.legend()
    fig.tight_layout()
    fig.savefig("Plots/Threshold/Attack2403/NetFlow/Xmas/OnlyXmasAttack.Scatter."+  str(systemId)+ ".SYN.png", dpi=300)
    plt.close()
