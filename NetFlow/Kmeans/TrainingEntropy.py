...
def trainingEntropy(silkFile, start, stop, systemId, frequency, interval):
    df = getEntropyData(silkFile, start, stop, frequency, interval)
    df.to_pickle("NetFlow/Kmeans/RawData/TrainingDataEntropy.attack."+str(attackDate)+ "."+str(systemId)+ ".pkl")
    #df = pd.read_pickle("NetFlow/Kmeans/RawData/TrainingDataEntropy.attack."+str(attackDate)+ "."+str(systemId)+ ".pkl")
    timeStamps, measurements =structureDataEntropy(df)

    kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(measurements)
    pickle.dump(kmeans, open("NetFlow/Kmeans/Models/MLmodelEntropy.attack."+str(attackDate)+ "."+str(systemId)+ ".pkl", 'wb'))
