...
def training(silkFile, start, stop, systemId, frequency, interval):
    ...
    '''df = getData(silkFile)
    df.to_pickle("NetFlow/Kmeans/RawData/TrainingData."+str(systemId)+ ".pkl")'''
    df = pd.read_pickle("NetFlow/Kmeans/RawData/TrainingData."+str(systemId)+ ".pkl")
    ...
    entropy_df.to_pickle("NetFlow/Kmeans/RawData/TrainingDataEntropy."+str(systemId)+ ".pkl")'''
    entropy_df = pd.read_pickle("NetFlow/Kmeans/RawData/TrainingDataEntropy."+str(systemId)+ ".pkl")   
    ...
    trainingSet.to_pickle("NetFlow/Kmeans/RawData/TrainingDataCombined."+str(systemId)+ ".pkl")
    print(trainingSet.head)

    kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(trainingSet)
    pickle.dump(kmeans, open("NetFlow/Kmeans/Models/MLmodelCombined."+str(systemId)+ ".pkl", 'wb'))
