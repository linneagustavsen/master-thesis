...
def plotKmeansFields(start, stop, clusterFrequency, systemId, attackDate):
    ...
    for i in range(math.ceil(intervalTime)):
        ...
        cluster0 = pd.read_csv("Calculations0803/Kmeans/NetFlow/Cluster0.attack."+str(attackDate)+ ".stopTime." + stopTime.strftime("%H.%M.%S")+ "."+ str(systemId)+ ".csv")
        cluster1 = pd.read_csv("Calculations0803/Kmeans/NetFlow/Cluster1.attack."+str(attackDate)+ ".stopTime" + stopTime.strftime("%H.%M.%S")+ "."+ str(systemId)+ ".csv")
    ...
    axs[1].legend()
    fig.tight_layout()
    fig.savefig("Plots/Kmeans/Attack0803/NetFlow/Fields/Packets."+  str(systemId)+ ".png", dpi=300)
    plt.close()
